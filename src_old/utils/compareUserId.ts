...
async function compareUserId<T extends keyof Database['public']['Tables']>(
  current_user: string,
  table: T
): Promise<Tables<T>[] | null> {
  if (table === 'user_base') {
    const { data, error } = await supabase.from(table).select('*').eq('id', current_user);
    ...
    const { data, error } = await supabase.from(table).select('*').eq('profile_id', current_user);
