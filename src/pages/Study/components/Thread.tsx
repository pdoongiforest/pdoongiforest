...
function Thread() {
  ...
  const targetThread = threadData.find(
    (thread) => thread.board_id == id
  );
      ...
      const replies = await Promise.all(
        threadData.map(async (thread) => {
          ...
          return [
            thread.thread_id,
            data || [],
          ] as [string, ReplyWithUser[]];
        })
      );

      const replyMap =
        Object.fromEntries(replies);
  ...
  // 개별 쓰레드에 맞는 댓글만 업데이트
  const setThreadReplyData = async (
    thread_id: string
  ) => {
    const { data, error } = await supabase
      .from('thread_reply')
    ...
    <>
      <div className={S.layout}>
        ...
        <div className={S.member}>
          <div className={S.recentlyUserTitle}>
            Channel Member
          </div>
