...
function Admin() {
  ...
  useEffect(() => {
    const fetchApproveUser = async () => {
      ...
      if (certificateFileError)
        console.error('수료증 가져오기 실패');
      if (!data) return;

      const result = await Promise.all(
        data.map(async (d) => {
          const matchFile = certificateFile?.find(
            (c) => c.image.split('.')[0] === d.id
          );
          if (!matchFile) {
            return {
              id: d.id,
  ...
  const updateApprove = async () => {
    ...
    const newApproveList = approveList.filter(
    ...
    setCertificateFile(
      newApproveList[0].certificateFile
    );
    setActiveId(newApproveList[0].id);
  };

  const list = approveList?.map(
    ({ name, id, certificateFile }) => (
      <button
        key={id}
  ...
  return (
    <div className={S.container}>
      ...
      <div className={S.confirmList}>
        <div className={S.userList}>{list}</div>
